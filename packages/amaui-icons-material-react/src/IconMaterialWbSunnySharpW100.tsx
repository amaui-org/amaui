import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialWbSunnySharpW100 = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='WbSunnySharpW100'
      short_name='WbSunny'

      {...props}
    >
      <path d="M11.65 3.85V1.65H12.35V3.85ZM11.65 22.35V20.15H12.35V22.35ZM20.15 12.35V11.65H22.35V12.35ZM1.65 12.35V11.65H3.85V12.35ZM18.2 6.3 17.7 5.8 19.05 4.45 19.55 4.95ZM4.95 19.55 4.45 19.05 5.8 17.7 6.3 18.2ZM19.05 19.55 17.7 18.2 18.2 17.7 19.55 19.05ZM5.8 6.3 4.45 4.95 4.95 4.45 6.3 5.8ZM12 16.7Q10.05 16.7 8.675 15.325Q7.3 13.95 7.3 12Q7.3 10.05 8.675 8.675Q10.05 7.3 12 7.3Q13.95 7.3 15.325 8.675Q16.7 10.05 16.7 12Q16.7 13.95 15.325 15.325Q13.95 16.7 12 16.7ZM12 16Q13.675 16 14.838 14.837Q16 13.675 16 12Q16 10.325 14.838 9.162Q13.675 8 12 8Q10.325 8 9.163 9.162Q8 10.325 8 12Q8 13.675 9.163 14.837Q10.325 16 12 16ZM12 12Q12 12 12 12Q12 12 12 12Q12 12 12 12Q12 12 12 12Q12 12 12 12Q12 12 12 12Q12 12 12 12Q12 12 12 12Z"/>
    </Icon>
  );
});

IconMaterialWbSunnySharpW100.displayName = 'AmauiIconMaterialWbSunnySharpW100';

export default IconMaterialWbSunnySharpW100;
