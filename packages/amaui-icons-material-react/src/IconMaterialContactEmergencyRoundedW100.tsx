import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialContactEmergencyRoundedW100 = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='ContactEmergencyRoundedW100'
      short_name='ContactEmergency'

      {...props}
    >
      <path d="M17.65 9.6v1.65q0 .15.1.25t.25.1q.15 0 .25-.1t.1-.25V9.6l1.425.825q.125.075.263.037.137-.037.212-.162.075-.125.038-.263-.038-.137-.163-.212L18.7 9l1.425-.825q.125-.075.163-.213.037-.137-.038-.262t-.212-.163q-.138-.037-.263.038L18.35 8.4V6.75q0-.15-.1-.25T18 6.4q-.15 0-.25.1t-.1.25V8.4l-1.425-.825q-.125-.075-.262-.038-.138.038-.213.163-.075.125-.037.262.037.138.162.213L17.3 9l-1.425.825q-.125.075-.162.212-.038.138.037.263t.213.162q.137.038.262-.037ZM2.8 19.2q-.65 0-1.075-.425Q1.3 18.35 1.3 17.7V6.3q0-.65.425-1.075Q2.15 4.8 2.8 4.8h18.4q.65 0 1.075.425.425.425.425 1.075v11.4q0 .65-.425 1.075-.425.425-1.075.425Zm11.65-.7h6.75q.3 0 .55-.25.25-.25.25-.55V6.3q0-.3-.25-.55-.25-.25-.55-.25H2.8q-.3 0-.55.25Q2 6 2 6.3v11.4q0 .3.25.55.25.25.55.25h.75Q4.6 17.3 6 16.55q1.4-.75 3-.75t3 .75q1.4.75 2.45 1.95ZM9 14.3q.975 0 1.663-.688.687-.687.687-1.662 0-.975-.687-1.663Q9.975 9.6 9 9.6q-.975 0-1.662.687-.688.688-.688 1.663 0 .975.688 1.662.687.688 1.662.688Zm-4.45 4.2h8.9q-.875-.95-2.025-1.475Q10.275 16.5 9 16.5t-2.425.525q-1.15.525-2.025 1.475ZM9 13.6q-.675 0-1.162-.487-.488-.488-.488-1.163t.488-1.163Q8.325 10.3 9 10.3q.675 0 1.163.487.487.488.487 1.163t-.487 1.163Q9.675 13.6 9 13.6Zm3-1.6Z"/>
    </Icon>
  );
});

IconMaterialContactEmergencyRoundedW100.displayName = 'AmauiIconMaterialContactEmergencyRoundedW100';

export default IconMaterialContactEmergencyRoundedW100;
