import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialPlaylistPlayRoundedW100 = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='PlaylistPlayRoundedW100'
      short_name='PlaylistPlay'

      {...props}
    >
      <path d="M4 7.35Q3.85 7.35 3.75 7.25Q3.65 7.15 3.65 7Q3.65 6.85 3.75 6.75Q3.85 6.65 4 6.65H14Q14.15 6.65 14.25 6.75Q14.35 6.85 14.35 7Q14.35 7.15 14.25 7.25Q14.15 7.35 14 7.35ZM4 11.35Q3.85 11.35 3.75 11.25Q3.65 11.15 3.65 11Q3.65 10.85 3.75 10.75Q3.85 10.65 4 10.65H14Q14.15 10.65 14.25 10.75Q14.35 10.85 14.35 11Q14.35 11.15 14.25 11.25Q14.15 11.35 14 11.35ZM4 15.35Q3.85 15.35 3.75 15.25Q3.65 15.15 3.65 15Q3.65 14.85 3.75 14.75Q3.85 14.65 4 14.65H10Q10.15 14.65 10.25 14.75Q10.35 14.85 10.35 15Q10.35 15.15 10.25 15.25Q10.15 15.35 10 15.35ZM16.7 19.425Q16.525 19.55 16.338 19.45Q16.15 19.35 16.15 19.15V14.85Q16.15 14.65 16.338 14.55Q16.525 14.45 16.7 14.575L19.925 16.7Q20.1 16.8 20.1 17Q20.1 17.2 19.925 17.3Z"/>
    </Icon>
  );
});

IconMaterialPlaylistPlayRoundedW100.displayName = 'AmauiIconMaterialPlaylistPlayRoundedW100';

export default IconMaterialPlaylistPlayRoundedW100;
