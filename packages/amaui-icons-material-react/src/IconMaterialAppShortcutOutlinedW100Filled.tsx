import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialAppShortcutOutlinedW100Filled = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='AppShortcutOutlinedW100Filled'
      short_name='AppShortcut'

      {...props}
    >
      <path d="M21 10.35 20.575 9.425 19.65 9 20.575 8.575 21 7.65 21.425 8.575 22.35 9 21.425 9.425ZM16 15.05 15.05 12.95 12.95 12 15.05 11.05 16 8.95 16.95 11.05 19.05 12 16.95 12.95ZM21 16.35 20.575 15.425 19.65 15 20.575 14.575 21 13.65 21.425 14.575 22.35 15 21.425 15.425ZM7.8 21.7Q7.175 21.7 6.738 21.262Q6.3 20.825 6.3 20.2V3.8Q6.3 3.175 6.738 2.737Q7.175 2.3 7.8 2.3H16.2Q16.825 2.3 17.262 2.737Q17.7 3.175 17.7 3.8V6.9H17V5.35H7V18.65H17V17.1H17.7V20.2Q17.7 20.825 17.262 21.262Q16.825 21.7 16.2 21.7Z"/>
    </Icon>
  )
});

export default IconMaterialAppShortcutOutlinedW100Filled;
