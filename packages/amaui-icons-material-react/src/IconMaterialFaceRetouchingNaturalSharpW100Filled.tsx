import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialFaceRetouchingNaturalSharpW100Filled = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='FaceRetouchingNaturalSharpW100Filled'
      short_name='FaceRetouchingNatural'

      {...props}
    >
      <path d="M12 20.7Q10.2 20.7 8.613 20.012Q7.025 19.325 5.85 18.15Q4.675 16.975 3.987 15.387Q3.3 13.8 3.3 12Q3.3 10.2 3.987 8.612Q4.675 7.025 5.85 5.85Q7.025 4.675 8.613 3.987Q10.2 3.3 12 3.3Q12.525 3.3 13 3.35Q13.475 3.4 13.875 3.5L11.625 4.5L17.1 6.925L18.425 9.95Q16.175 10.175 13.825 9.312Q11.475 8.45 9.75 6.3Q8.7 8.8 7.125 10.025Q5.55 11.25 4 11.85Q4 15.325 6.338 17.663Q8.675 20 12 20Q15.4 20 17.7 17.6Q20 15.2 20 12Q20 11.725 19.988 11.5Q19.975 11.275 19.95 11.15L20.475 10.025Q20.6 10.475 20.65 10.988Q20.7 11.5 20.7 12Q20.7 13.8 20.013 15.387Q19.325 16.975 18.15 18.15Q16.975 19.325 15.388 20.012Q13.8 20.7 12 20.7ZM9 14.25Q8.475 14.25 8.113 13.887Q7.75 13.525 7.75 13Q7.75 12.475 8.113 12.113Q8.475 11.75 9 11.75Q9.525 11.75 9.887 12.113Q10.25 12.475 10.25 13Q10.25 13.525 9.887 13.887Q9.525 14.25 9 14.25ZM15 14.25Q14.475 14.25 14.113 13.887Q13.75 13.525 13.75 13Q13.75 12.475 14.113 12.113Q14.475 11.75 15 11.75Q15.525 11.75 15.887 12.113Q16.25 12.475 16.25 13Q16.25 13.525 15.887 13.887Q15.525 14.25 15 14.25ZM19.5 7.4 18.6 5.4 16.6 4.5 18.6 3.6 19.5 1.6 20.4 3.6 22.4 4.5 20.4 5.4Z"/>
    </Icon>
  );
});

IconMaterialFaceRetouchingNaturalSharpW100Filled.displayName = 'AmauiIconMaterialFaceRetouchingNaturalSharpW100Filled';

export default IconMaterialFaceRetouchingNaturalSharpW100Filled;
