import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialNetworkWifi2BarRoundedW100 = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='NetworkWifi2BarRoundedW100'
      short_name='NetworkWifi2Bar'

      {...props}
    >
      <path d="M7.475 13.625Q8.425 12.925 9.575 12.537Q10.725 12.15 12 12.15Q13.275 12.15 14.425 12.537Q15.575 12.925 16.525 13.625L21.1 9.05Q19.15 7.575 16.838 6.787Q14.525 6 12 6Q9.475 6 7.162 6.787Q4.85 7.575 2.9 9.05ZM12 18.85Q11.85 18.85 11.725 18.8Q11.6 18.75 11.475 18.625L2.35 9.5Q2.15 9.3 2.163 9.012Q2.175 8.725 2.4 8.55Q4.4 6.975 6.825 6.137Q9.25 5.3 12 5.3Q14.75 5.3 17.175 6.137Q19.6 6.975 21.6 8.55Q21.825 8.725 21.838 9.012Q21.85 9.3 21.65 9.5L12.525 18.625Q12.4 18.75 12.275 18.8Q12.15 18.85 12 18.85Z"/>
    </Icon>
  );
});

IconMaterialNetworkWifi2BarRoundedW100.displayName = 'AmauiIconMaterialNetworkWifi2BarRoundedW100';

export default IconMaterialNetworkWifi2BarRoundedW100;
