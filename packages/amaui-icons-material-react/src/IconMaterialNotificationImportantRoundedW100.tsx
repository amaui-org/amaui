import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialNotificationImportantRoundedW100 = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='NotificationImportantRoundedW100'
      short_name='NotificationImportant'

      {...props}
    >
      <path d="M12 13.5Q12.15 13.5 12.25 13.4Q12.35 13.3 12.35 13.15V9.1Q12.35 8.95 12.25 8.85Q12.15 8.75 12 8.75Q11.85 8.75 11.75 8.85Q11.65 8.95 11.65 9.1V13.15Q11.65 13.3 11.75 13.4Q11.85 13.5 12 13.5ZM12 15.75Q12.225 15.75 12.363 15.6Q12.5 15.45 12.5 15.25Q12.5 15.025 12.363 14.887Q12.225 14.75 12 14.75Q11.8 14.75 11.65 14.887Q11.5 15.025 11.5 15.25Q11.5 15.45 11.65 15.6Q11.8 15.75 12 15.75ZM5 18.65Q4.85 18.65 4.75 18.55Q4.65 18.45 4.65 18.3Q4.65 18.15 4.75 18.05Q4.85 17.95 5 17.95H6.65V10.3Q6.65 8.425 7.9 6.887Q9.15 5.35 11.15 5V4.3Q11.15 3.925 11.387 3.687Q11.625 3.45 12 3.45Q12.375 3.45 12.613 3.687Q12.85 3.925 12.85 4.3V5Q14.85 5.35 16.1 6.887Q17.35 8.425 17.35 10.3V17.95H19Q19.15 17.95 19.25 18.05Q19.35 18.15 19.35 18.3Q19.35 18.45 19.25 18.55Q19.15 18.65 19 18.65ZM12 11.8Q12 11.8 12 11.8Q12 11.8 12 11.8Q12 11.8 12 11.8Q12 11.8 12 11.8ZM12 21.5Q11.4 21.5 10.95 21.075Q10.5 20.65 10.5 20H13.5Q13.5 20.65 13.075 21.075Q12.65 21.5 12 21.5ZM7.35 17.95H16.65V10.3Q16.65 8.375 15.288 7.012Q13.925 5.65 12 5.65Q10.075 5.65 8.713 7.012Q7.35 8.375 7.35 10.3Z"/>
    </Icon>
  );
});

IconMaterialNotificationImportantRoundedW100.displayName = 'AmauiIconMaterialNotificationImportantRoundedW100';

export default IconMaterialNotificationImportantRoundedW100;
