import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialTimeAutoSharpW700Filled = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='TimeAutoSharpW700Filled'
      short_name='TimeAuto'

      {...props}
    >
      <path d="M8 17.525H9.85L10.425 15.775H13.525L14.1 17.525H16L13 9.075H11ZM10.9 14.325 12 11.025 13.075 14.325ZM8.225 3V0.225H15.775V3ZM12 23.525Q9.95 23.525 8.163 22.75Q6.375 21.975 5.037 20.65Q3.7 19.325 2.925 17.538Q2.15 15.75 2.15 13.7Q2.15 11.65 2.925 9.862Q3.7 8.075 5.037 6.737Q6.375 5.4 8.163 4.625Q9.95 3.85 12 3.85Q13.6 3.85 15.125 4.35Q16.65 4.85 17.925 5.825L19.625 4.125L21.6 6.1L19.9 7.8Q20.875 9.075 21.363 10.587Q21.85 12.1 21.85 13.7Q21.85 15.75 21.075 17.538Q20.3 19.325 18.962 20.65Q17.625 21.975 15.838 22.75Q14.05 23.525 12 23.525Z"/>
    </Icon>
  )
});

export default IconMaterialTimeAutoSharpW700Filled;
