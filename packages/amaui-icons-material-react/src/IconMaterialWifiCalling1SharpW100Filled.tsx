import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialWifiCalling1SharpW100Filled = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='WifiCalling1SharpW100Filled'
      short_name='WifiCalling1'

      {...props}
    >
      <path d="M16.5 11.6Q16.225 11.6 16.013 11.387Q15.8 11.175 15.8 10.9Q15.8 10.625 16.013 10.412Q16.225 10.2 16.5 10.2Q16.775 10.2 16.988 10.412Q17.2 10.625 17.2 10.9Q17.2 11.175 16.988 11.387Q16.775 11.6 16.5 11.6ZM14 8.9 13.55 8.4Q14.2 7.9 14.95 7.6Q15.7 7.3 16.5 7.3Q17.3 7.3 18.05 7.6Q18.8 7.9 19.45 8.4L19 8.9Q18.45 8.475 17.812 8.238Q17.175 8 16.5 8Q15.825 8 15.188 8.238Q14.55 8.475 14 8.9ZM11.9 6.75 11.4 6.25Q12.45 5.3 13.763 4.8Q15.075 4.3 16.5 4.3Q17.925 4.3 19.238 4.8Q20.55 5.3 21.6 6.25L21.1 6.75Q20.15 5.9 18.963 5.45Q17.775 5 16.5 5Q15.225 5 14.038 5.45Q12.85 5.9 11.9 6.75ZM18.625 19.55Q16.2 19.575 13.662 18.275Q11.125 16.975 9.05 14.913Q6.975 12.85 5.663 10.312Q4.35 7.775 4.35 5.35Q4.35 5.1 4.363 4.825Q4.375 4.55 4.425 4.3H8.375L9.2 8.075L6.55 10.5Q7.95 12.85 9.675 14.55Q11.4 16.25 13.75 17.55L16.3 14.875L19.65 15.575V19.475Q19.425 19.525 19.138 19.538Q18.85 19.55 18.625 19.55Z"/>
    </Icon>
  );
});

IconMaterialWifiCalling1SharpW100Filled.displayName = 'AmauiIconMaterialWifiCalling1SharpW100Filled';

export default IconMaterialWifiCalling1SharpW100Filled;
