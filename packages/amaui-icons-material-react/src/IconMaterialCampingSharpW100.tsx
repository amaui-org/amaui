import React from 'react';

import { Icon } from '@amaui/ui-react';

const IconMaterialCampingSharpW100 = React.forwardRef((props: any, ref) => {

  return (
    <Icon
      ref={ref}

      name='CampingSharpW100'
      short_name='Camping'

      {...props}
    >
      <path d="M3.3 21.35v-3.6l8.275-11.125-.95-1.25.575-.425.8 1.1.825-1.1.55.425-.925 1.25L20.7 17.75v3.6ZM12 7.225l-8 10.75v2.675h3.325L12 14.125l4.675 6.525H20v-2.675ZM8.2 20.65h7.6L12 15.325Zm3.8-6.525 4.675 6.525L12 14.125 7.325 20.65Z"/>
    </Icon>
  );
});

IconMaterialCampingSharpW100.displayName = 'AmauiIconMaterialCampingSharpW100';

export default IconMaterialCampingSharpW100;
