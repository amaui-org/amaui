import React from 'react';

import { Icon, IIcon } from '@amaui/ui-react';

const IconMaterialSdW100 = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='SdW100'

      short_name='Sd'

      viewBox='0 0 24 24'

      {...props}
    >
      <path d="M7.4 14.6h3q.275 0 .438-.162Q11 14.275 11 14v-1.75q0-.275-.162-.438-.163-.162-.438-.162H7.5V10.1h2.8v.5h.7V10q0-.275-.162-.438-.163-.162-.438-.162h-3q-.275 0-.437.162Q6.8 9.725 6.8 10v1.75q0 .275.163.438.162.162.437.162h2.9v1.55H7.5v-.5h-.7v.6q0 .275.163.438.162.162.437.162Zm5.6 0h3.1q.45 0 .775-.325t.325-.775v-3q0-.45-.325-.775T16.1 9.4H13Zm.7-.7v-3.8h2.4q.15 0 .275.125.125.125.125.275v3q0 .15-.125.275-.125.125-.275.125Zm-8.9 4.8q-.65 0-1.075-.425Q3.3 17.85 3.3 17.2V6.8q0-.65.425-1.075Q4.15 5.3 4.8 5.3h14.4q.65 0 1.075.425.425.425.425 1.075v10.4q0 .65-.425 1.075-.425.425-1.075.425ZM4 6v12V6Zm15.2 12q.3 0 .55-.25.25-.25.25-.55V6.8q0-.3-.25-.55Q19.5 6 19.2 6H4.8q-.3 0-.55.25Q4 6.5 4 6.8v10.4q0 .3.25.55.25.25.55.25Z"/>
    </Icon>
  );
});

IconMaterialSdW100.displayName = 'AmauiIconMaterialSdW100';

export default IconMaterialSdW100;
