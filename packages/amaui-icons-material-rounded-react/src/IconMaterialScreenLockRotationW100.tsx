import React from 'react';

import { Icon, IIcon } from '@amaui/ui-react';

const IconMaterialScreenLockRotationW100 = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='ScreenLockRotationW100'

      short_name='ScreenLockRotation'

      viewBox='0 0 24 24'

      {...props}
    >
      <path d="M14.925 17.475q-.3 0-.575-.112-.275-.113-.5-.313l-6.9-6.9q-.2-.225-.312-.5-.113-.275-.113-.575 0-.3.113-.588Q6.75 8.2 6.95 8l2.8-2.8q.225-.225.5-.325t.575-.1q.3 0 .588.1.287.1.487.325l.85.85q.05.05.075.112.025.063.025.125 0 .063-.025.138-.025.075-.075.125-.125.125-.25.125t-.25-.125l-.85-.85q-.225-.225-.575-.225t-.575.225l-2.8 2.8q-.225.225-.225.575t.225.575l6.9 6.9q.225.225.575.225t.575-.225l2.8-2.8q.225-.225.225-.575T18.3 12.6l-.75-.75q-.1-.1-.112-.237-.013-.138.112-.263t.25-.125q.125 0 .25.125l.75.75q.225.2.325.487.1.288.1.588t-.1.575q-.1.275-.325.5l-2.8 2.8q-.2.2-.487.313-.288.112-.588.112Zm-1.425 4.35q-.15.025-.3.025h-.3q-2.175 0-4.088-.812-1.912-.813-3.362-2.213-1.45-1.4-2.312-3.288Q2.275 13.65 2.2 11.5q0-.15.1-.25t.25-.1q.15 0 .25.1t.1.25q.125 4.025 3.038 6.837Q8.85 21.15 12.9 21.15L9.45 17.7q-.1-.1-.112-.238-.013-.137.112-.262t.25-.125q.125 0 .25.125l3.825 3.825q.275.275.2.513-.075.237-.475.287ZM15.9 9q-.35 0-.625-.275T15 8.1V5.5q0-.35.275-.625T15.9 4.6h.125v-1q0-.725.488-1.213Q17 1.9 17.725 1.9t1.213.487q.487.488.487 1.213v1h.125q.35 0 .6.275t.25.625v2.6q0 .35-.275.625T19.5 9Zm.825-4.4h2v-1q0-.425-.287-.713-.288-.287-.713-.287t-.712.287q-.288.288-.288.713Zm-3.85 6.525Z"/>
    </Icon>
  );
});

IconMaterialScreenLockRotationW100.displayName = 'AmauiIconMaterialScreenLockRotationW100';

export default IconMaterialScreenLockRotationW100;
