import React from 'react';

import { Icon, IIcon } from '@amaui/ui-react';

const IconMaterialEdgesensorLowW100Filled = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='EdgesensorLowW100Filled'

      short_name='EdgesensorLow'

      viewBox='0 0 24 24'

      {...props}
    >
      <path d="M8.55 20.7q-.65 0-1.075-.425-.425-.425-.425-1.075V4.8q0-.65.425-1.075Q7.9 3.3 8.55 3.3h6.9q.65 0 1.075.425.425.425.425 1.075v14.4q0 .65-.425 1.075-.425.425-1.075.425Zm-.8-3.05h8.5V6.35h-8.5Zm-3.05-4.3q-.15 0-.25-.1t-.1-.25V8q0-.15.1-.25t.25-.1q.15 0 .25.1t.1.25v5q0 .15-.1.25t-.25.1Zm14.6 3q-.15 0-.25-.1t-.1-.25v-5q0-.15.1-.25t.25-.1q.15 0 .25.1t.1.25v5q0 .15-.1.25t-.25.1Z"/>
    </Icon>
  );
});

IconMaterialEdgesensorLowW100Filled.displayName = 'AmauiIconMaterialEdgesensorLowW100Filled';

export default IconMaterialEdgesensorLowW100Filled;
