import React from 'react';

import { Icon, IIcon } from '@amaui/ui-react';

const IconMaterialStadiumW100Filled = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='StadiumW100Filled'

      short_name='Stadium'

      viewBox='0 0 24 24'

      {...props}
    >
      <path d="M5.475 6.7q-.2.1-.387-.012Q4.9 6.575 4.9 6.35v-1.9q0-.225.188-.338.187-.112.387-.012l1.9.95q.125.05.175.137.05.088.05.213t-.05.212q-.05.088-.175.138Zm12 0q-.2.1-.387-.012-.188-.113-.188-.338v-1.9q0-.225.188-.338.187-.112.387-.012l1.9.95q.125.05.175.137.05.088.05.213t-.05.212q-.05.088-.175.138Zm-5.6-.8q-.2.1-.387-.013-.188-.112-.188-.337v-1.9q0-.225.188-.338.187-.112.387-.012l1.9.95q.125.05.175.137.05.088.05.213t-.05.212q-.05.088-.175.138ZM9.65 21.05q-1.475-.1-2.675-.313-1.2-.212-2.05-.512-.85-.3-1.312-.675-.463-.375-.463-.8v-8q0-.5.65-.925.65-.425 1.825-.738 1.175-.312 2.8-.5Q10.05 8.4 12 8.4q1.95 0 3.575.187 1.625.188 2.8.5 1.175.313 1.825.738.65.425.65.925v8q0 .425-.462.8-.463.375-1.313.675-.85.3-2.05.512-1.2.213-2.675.313V17.5q0-.325-.212-.538-.213-.212-.538-.212h-3.2q-.325 0-.537.212-.213.213-.213.538ZM12 12.4q2.5 0 4.788-.425 2.287-.425 3.362-1.15-.425-.65-2.537-1.188Q15.5 9.1 12 9.1q-3.5 0-5.612.537-2.113.538-2.538 1.188 1.075.725 3.075 1.15 2 .425 5.075.425Z"/>
    </Icon>
  );
});

IconMaterialStadiumW100Filled.displayName = 'AmauiIconMaterialStadiumW100Filled';

export default IconMaterialStadiumW100Filled;
