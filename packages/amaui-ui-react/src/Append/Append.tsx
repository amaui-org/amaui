...
const Append = (props_: any) => {
  ...
  const make = () => {
    ...
    if (refs.root.current && refs.element.current) {
      ...
      if (position === 'bottom' || position === 'top') {
        if (alignment === 'left') values_.x = rootX;

        if (!alignment || alignment === 'center') values_.x = rootX + ((rect.root.width - rect.element.width) / 2);

        if (alignment === 'right') values_.x = rootX + rect.root.width - rect.element.width;
      ...
      // Left
      if (position === 'left') {
        if (alignment === 'top') values_.y = rootY;

        if (!alignment || alignment === 'center') values_.y = rootY + ((rect.root.height - rect.element.height) / 2);

        if (alignment === 'bottom') values_.y = rootY + rect.root.height - rect.element.height;
  ...
  if (accelerated) {
    const multi = theme.direction === 'rtl' ? -1 : 1;
    const ppiHigh = isEnvironment('browser') && window.devicePixelRatio > 1;

    if (ppiHigh) style.transform = `translate3d(${values.x * multi}px, ${values.y}px, 0px)`;
    else style.transform = `translate(${values.x * multi}px, ${values.y}px)`;
  }
  else {
    style.top = values.y;
    style.left = values.x;
